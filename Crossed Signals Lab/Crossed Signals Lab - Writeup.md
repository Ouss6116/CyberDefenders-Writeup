...
then we get the key from Signal local state file, and decode it and recreat it as binay with powershell:
...
now we can work on SQLCipher key, by using the key in Signal config.json and the decrypted key in previous question, by using the following AES-GCM decryption python script:
